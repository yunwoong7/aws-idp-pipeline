# src/agent/chat_agent/prompt/__init__.py
...
class PromptLoader:
        ...
        for key, value in prompt_data.items():
            if isinstance(value, str):
                ...
                try:
                    ...
                    # Safe format: escape remaining single braces for JSON structures
                    safe_value = self._escape_json_braces(converted_value)
                    formatted_value = safe_value.format(**kwargs)
                    
                    # Restore escaped JSON braces
                    result[key] = self._restore_json_braces(formatted_value)
    ...
    def _escape_json_braces(self, text: str) -> str:
        """
        Escape JSON braces to prevent format() errors
        
        Args:
            text: Text to escape
            
        Returns:
            str: Text with JSON braces escaped
        """
        # Simple approach: escape all single braces by doubling them
        # Then restore legitimate format variables
        text = text.replace('{', '{{').replace('}', '}}')
        
        # Restore format variables: {{variable_name}} -> {variable_name}
        text = re.sub(r'\{\{([A-Za-z_][A-Za-z0-9_]*)\}\}', r'{\1}', text)
        
        return text
    
    def _restore_json_braces(self, text: str) -> str:
        """
        Restore JSON braces after formatting
        
        Args:
            text: Text to restore
            
        Returns:
            str: Text with JSON braces restored
        """
        # Convert double braces back to single braces
        text = text.replace('{{', '{').replace('}}', '}')
        return text
